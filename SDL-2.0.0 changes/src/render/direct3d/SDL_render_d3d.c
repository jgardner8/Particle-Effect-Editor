...
D3D_SetBlendMode(D3D_RenderData * data, int blendMode)
{
	...
	//MY IMPLEMENTATION, WORKS ON TRANSPARENT RENDER TARGETS. 
	//TODO: make an equivalent change to OpenGL renderer so program is cross-platform.
	//TODO: consider making it into a separate blend mode.
	case SDL_BLENDMODE_ADD:
        IDirect3DDevice9_SetRenderState(data->device, D3DRS_ALPHABLENDENABLE, TRUE);
        IDirect3DDevice9_SetRenderState(data->device, D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
        IDirect3DDevice9_SetRenderState(data->device, D3DRS_DESTBLEND, D3DBLEND_ONE);
        if (data->enableSeparateAlphaBlend) {
            IDirect3DDevice9_SetRenderState(data->device, D3DRS_SRCBLENDALPHA, D3DBLEND_ONE);
            IDirect3DDevice9_SetRenderState(data->device, D3DRS_DESTBLENDALPHA, D3DBLEND_ONE);
        }
        break;
